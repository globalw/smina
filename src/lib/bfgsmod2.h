		...
		const minimization_params& params)
{
	...
	VINA_U_FOR(step, params.maxiters)
	{
		...
		//irgendwie müssen wir hier die Gradientenberechnung rausbekommen...
		//alpha = accurate_line_search(f, n, x, g, f0, s, x_new, g_new, f1);
		fb = f(x_new, g);


		// es scheint so zu sein, als ob mit der funktion f(x,g) nicht nur der funktionswert
		// zurückgegeben wird, sondern auch der Gradient der function im punkt x berechnet wird

		//berechne den Nenner des Suchschrittes
		//fl gradnorm = scalar_product(g, g, n);
		fl gradnorm = vector_norm(g,n);
		// hier addieren wir den Suchschritt auf die bisherige configuration. Sie heisst
		// x_new und wurde vorher mit der configuration x gleichgesetzt.
		x = x_new;
		fl strech = (bnp1 - fb)/(gradnorm*gradnorm);
		x_new.increment(g, strech);
		//x_new.increment(g, 1.0);

		// Wegen der Architektur dieses Programmes nutzen wir die Norm des Gradienten
		// um konvergenz zu ermitteln... oder den abstand der Konfigurationen
		...
		if(res < 1e-3)
		{
			...
			x = x_new;
			tmp = bnp1;
			// in diesem Fall schieben wir die Niveauline um die hälfte nach unten
			bnp1 = bnp1 - fabs(bnp1 - bn) / 2.0;
			bn = tmp;
			step = 0;
		}

		if(step == params.maxiters - 1){
			x = x_orig;
			x_new = x;
			// divergenz ist eingetreten
			tmp = bnp1;
			bnp1 = bnp1 + fabs(bnp1 - bn) / 2.0;
			bn = tmp;
			step = 0;
		...
		if (fabs(bn-bnp1)<1e-2)
		{
			// globales minimum wurde approximiert
			step = params.maxiters;
			break;
		}
	}

	// wir bemerken hier, dass f0 zu fb verändert wurde. Wenn wir wieder die smina
	// methoden verwenden wollen sollten wir das hier wieder ändern! Aber jetzt erstmal mit fb
	/*
	if (!(fb <= f_orig))
	...
	*/
	 ...
	 *
	 *
	// we need to keep the naming as long as we dont know
	...
	VINA_U_FOR(step, params.maxiters)
	{
		...
		// wir behalten diese Zeile weil wir den neuen Gradienten auch brauchen
		alpha = accurate_line_search(f, n, x, g, f0, s, x_new, g_new, f1);

		//compute B, multiplier of previous gradient
		// hier berechen wir die norm des gradienten
		fl gradnorm = scalar_product(g, g, n);
		// Suchvektorschritt
		fb = f(x, g);
		vector_multiply(g_new,g, (bnp1 - fb)/(gradnorm), n);
		//subtract_change(x, g_new, n);
		//x_new(x);
		if(!(gradnorm >= 1e-5*1e-5))
		{
			// in diesem Fall schieben wir die Niveauline um die hälfte nach unten
			bnp1 = bnp1 - (bnp1 - bn) / 2.0;
			step = 0;
